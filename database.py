...
database.py
-----------
Lớp truy cập dữ liệu (Data Access Layer) cho ứng dụng Email Guard AI.
Sử dụng SQLite - không cần cài đặt server CSDL riêng, phù hợp cho đồ án.

Bảng dữ liệu:
    users  -> lưu tài khoản người dùng (mật khẩu đã băm, KHÔNG lưu plaintext)
    checks -> lưu lịch sử mỗi email đã được AI phân tích
"""
...
def get_connection():
    """Mở kết nối tới SQLite, bật hỗ trợ khóa ngoại và trả kết quả dạng dict-like."""
...
def init_db():
    """Khởi tạo bảng nếu chưa tồn tại. Gọi 1 lần khi ứng dụng khởi động."""
    conn = get_connection()
    cur = conn.cursor()
...
# ---------------------------------------------------------------------------
# CHECKS (lịch sử phân tích email)
# ---------------------------------------------------------------------------
